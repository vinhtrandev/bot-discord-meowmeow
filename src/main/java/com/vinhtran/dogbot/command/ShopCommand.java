...
public class ShopCommand implements Command {
    ...
    public void execute(MessageReceivedEvent event, String[] args) {
        ...
        if (args.length < 2) { showShop(event, serverId); return; }

        switch (args[1].toLowerCase()) {
            case "mua"      -> handleBuy(event, userId, serverId, args);
            case "trang_bi" -> handleEquip(event, userId, serverId, args);
            case "tui_do"   -> handleInventory(event, userId, serverId);
            default         -> showShop(event, serverId);
        }
    }

    // ── Hiển thị cửa hàng ────────────────────────────────────────────────

    private void showShop(MessageReceivedEvent event, String serverId) {
        List<ShopItem> items = shopService.getAllItems(serverId);
        ...
        items.stream().filter(i -> i.getType().equals("SKIN_BAI")).forEach(i ->
                skinSb.append(i.getEmoji()).append(" **").append(i.getName()).append("**")
                        .append(" — ").append(i.getPrice() == 0 ? "Miễn phí" : i.getPrice() + " 🪙")
    ...
    private void handleBuy(MessageReceivedEvent event, String userId,
                           String serverId, String[] args) {
        if (args.length < 3) {
            event.getChannel().sendMessage("❌ Dùng: `!shop mua <item_id>`").queue();
            return;
        }
        try {
            shopService.buyItem(userId, serverId, args[2]);
    ...
    private void handleEquip(MessageReceivedEvent event, String userId,
                             String serverId, String[] args) {
        if (args.length < 3) {
            event.getChannel().sendMessage("❌ Dùng: `!shop trang_bi <item_id>`").queue();
            return;
        }
        try {
            shopService.equipItem(userId, serverId, args[2]);
            event.getChannel().sendMessageEmbeds(new EmbedBuilder()
    ...
    private void handleInventory(MessageReceivedEvent event, String userId, String serverId) {
        try {
            List<UserInventory> inv = shopService.getInventory(userId, serverId);
            ...
            event.getChannel().sendMessageEmbeds(new EmbedBuilder()
                    ...
                    .setFooter("✅ = Đang trang bị  |  !shop trang_bi <item_id> để trang bị")
